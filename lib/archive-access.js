...
module.exports = function(setRegion, kinesisClient, dynamoDB) {
	...
	common.getArchiveSettingsForStream(streamName, streamModeCache, this.kinesisClient, true, function(err) {
		...
		if (streamModeCache[streamName][RECOVERY_MODE_TAG_NAME] == RECOVERY_MODE_ALL) {
		    // add the sort key for the supplied sequence numbers if all
		    // data is captured
		...
		async.whilst(function() {
			...
			if (err) {
				...
				// this is the last page of query results, so
				// mark that we are done to the async iterator
				moreRecords = false;
			    } else {
				// more records to come, so bind this key into
				// the exclusive start key for the next query
