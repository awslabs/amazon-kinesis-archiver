...
module.exports = function(setRegion, kinesisClient, dynamoDB) {
    ...
    scanArchive = function(streamName, sequenceStart, lastUpdateDateStart, recordLimit, recordCallback, finalCallback) {
	var streamModeCache = {};
	common.getArchiveSettingsForStream(streamName, streamModeCache, this.kinesisClient, true, function(err) {
		...
		if (lastUpdateDateStart) {
		    params.ScanFilter[lastUpdateDateName] = {
			AttributeValueList : [ {
			    S : lastUpdateDateStart
			} ],
			ComparisonOperator : "GE"
		    };
		}
