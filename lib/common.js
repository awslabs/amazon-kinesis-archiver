...
function getArchiveSettingsForStream(streamName, streamModeCache, kinesisClient, forceInvalidate, callback) {
    if (!(streamName in streamModeCache) || forceInvalidate === true) {
        if (debug) {
            if (forceInvalidate) {
                console.log("Force resolution of Archive Mode for Stream " + streamName)
            } else {
                console.log("Building Archive Mode cache for Stream " + streamName);
            }
        }
        ...
        kinesisClient.listTagsForStream({
            ...
            if (err) {
                ...
                if (debug) {
                    console.log("Created Cache Entry: " + JSON.stringify(cacheEntry));
                }
        ...
        // go!
        if (debug) {
            console.log("Found previously cached information for " + streamName);
        }
        if (callback) {
            callback();
        }
