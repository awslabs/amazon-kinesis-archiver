...
module.exports = function() {
	...
	if (err) {
	    ...
	    common.createTables(_setRegion, _streamName, _archiveMode, _readIOPS, _writeIOPS, _ttlSeconds,
		    function(err) {
			if (err) {
			    console.log(err);
			    process.exit(ERROR);
    ...
    runInteractiveSetup = function() {
	qs = [ q_region, q_streamName, q_archiveMode, q_setTTL, q_readIOPS, q_writeIOPS ];
	async.waterfall(qs, last);
    };

    runSetup = function(streamName, region, archiveMode, readIOPS, writeIOPS, ttlSeconds, callback) {
	common.createTables(region, streamName, archiveMode, readIOPS, writeIOPS, ttlSeconds, function(err) {
	    if (callback) {
		callback(err);
	    }
	});
